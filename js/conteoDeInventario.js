...
function validarCodigoBarras(input) {
  ...
  if (codBarra) {
    const params = "?pCodigoLectura=" + codBarra;
      ...
      .then((result) => {
        if (result.codigo[0].CodigoArticulo != "ND") {
          ...
          Swal.fire({
            icon: "warning",
            title:
              "El código ingresado no es válido. Verifíquelo nuevamente o consulte a su supervisor.",
...
async function resumen() {
  ...
  try {
    ...
    const response = await fetch(
      env.API_URL + "wmsresumeninventario" + params,
      myInit
    );
    if (!response.ok)
    ...
    mostrarMensajeError(
      "Hubo un error al cargar los datos. Inténtalo de nuevo más tarde.",
      encabezado.length
    );
    labelCantidadRegistros.textContent = "Cantidad de registros: 0";
  }
  ...
  function renderizarDatos(datos) {
    ...
    paginaDatos.forEach((dato) => {
      let filaHTML = `
                <tr>
                    <td style="text-align: center;">
                        <h5 style="text-align: center;color: #f56108;">${
                          dato.ARTICULO
                        }</h5>
      ...
      if (dato.CONCILIADO === "N") {
        filaHTML = filaHTML.replace(
          "<tr>",
          '<tr style="color: #f56108; font-weight: bold;">'
        );
      }
  ...
  function actualizarPaginacion() {
    ...
    btnAnterior.onmouseover = () =>
      (btnAnterior.style.backgroundColor = "#7e8180"); // Hover
    btnAnterior.onmouseleave = () =>
      (btnAnterior.style.backgroundColor = "#7e8180"); // Volver al original
    ...
    btnVerPaginas.onmouseover = () =>
      (btnVerPaginas.style.backgroundColor = "#7e8180");
    btnVerPaginas.onmouseleave = () =>
      (btnVerPaginas.style.backgroundColor = "#7e8180");
  ...
  function ordenarTabla(indiceColumna) {
    ...
    const ordenAscendente =
      !thead.getAttribute("data-order") ||
      thead.getAttribute("data-order") === "desc";
    filas.sort((a, b) => {
...
async function resumenGeneral() {
  ...
  function renderizarDatos(datos) {
    ...
    pageData.forEach((dato) => {
      ...
      celdaArticulo.innerHTML = `
                ...
                <h6 style="margin: 0; text-align: center; color: #000000de">${
    ...
    if (paginaActual === totalPaginas) {
      ...
      [totalGeneralCant, totalGeneralExist, totalGeneralDif].forEach(
        (total) => {
  ...
  function actualizarPaginacion() {
    ...
    btnAnterior.style.transition = "0.3s"; // Animación suave
    btnAnterior.onmouseover = () =>
      (btnAnterior.style.backgroundColor = "#218838"); // Hover
    btnAnterior.onmouseleave = () =>
      (btnAnterior.style.backgroundColor = "#7e8180"); // Volver al original
    paginacion.appendChild(btnAnterior);
    ...
    btnSiguiente.style.transition = "0.3s";
    btnSiguiente.onmouseover = () =>
      (btnSiguiente.style.backgroundColor = "#218838");
    btnSiguiente.onmouseleave = () =>
      (btnSiguiente.style.backgroundColor = "#7e8180");
    paginacion.appendChild(btnSiguiente);
  }
...
function confirmarGuardadoParcialLectura() {
  ...
  if (tabla && tabla.rows.length > 2 && fechaInv.length > 0) {
    ...
    Swal.fire({
      icon: "warning",
      title:
        "Está intentando guardar sin antes realizar una lectura válida del inventario. Recuerde verificar la fecha asignadas al inventario.",
...
function guardaParcialMenteLectura() {
  ...
  const sendChunk = (chunk, index) => {
    const params = `?pUsuario=${pUsuario}&pBodega=${pBodega}&pEstado=${pEstado}&pFecha=${pFecha}&jsonDetalles=${encodeURIComponent(
    ...
    fetch(env.API_URL + "wmsguardaconteoinv/I" + params, myInit)
      ...
      .then((result) => {
        if (result.msg === "SUCCESS") {
          ...
          console.error(
            `Error al guardar bloque ${index + 1}:`,
            result.message
          );
        }
      })
...
function limpiarResultadoGeneral() {
  ...
  console.log(
    "Tabla de lectura y campos relacionados limpiados correctamente."
  );
  crearNuevaFila();
}
/////////////////////////////////////////////////////////////////////
...
function descargarExcel() {
  ...
  const totales = {
    CONTEO: jsonData.reduce(
      (sum, item) => sum + (parseFloat(item.CONTEO) || 0),
      0
    ),
    EXISTENCIA: jsonData.reduce(
      (sum, item) => sum + (parseFloat(item.EXISTENCIA) || 0),
      0
    ),
    DIFERENCIA: jsonData.reduce(
      (sum, item) => sum + (parseFloat(item.DIFERENCIA) || 0),
      0
    ),
  };
...
function descargarPDF() {
  ...
  const totales = {
    CONTEO: datosOriginales.reduce(
      (sum, item) => sum + parseFloat(item.CONTEO || 0),
      0
    ),
    EXISTENCIA: datosOriginales.reduce(
      (sum, item) => sum + parseFloat(item.EXISTENCIA || 0),
      0
    ),
    DIFERENCIA: datosOriginales.reduce(
      (sum, item) => sum + parseFloat(item.DIFERENCIA || 0),
      0
    ),
  };
