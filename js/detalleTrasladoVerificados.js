...
function getTipoTransaccion() {
  ...
  const checkedCount =
    (traslado.checked ? 1 : 0) +
    (venta.checked ? 1 : 0) +
    (ordenDeCompras.checked ? 1 : 0);
...
function mostrarTotalesGlobales() {
  const totalesContainer =
    document.getElementById("totalesContainer") ||
  ...
  totalesContainer.innerHTML = `
    <div style="font-size: 20px; text-align: right; margin-top: 20px; padding: 10px;">
      ...
      <p id="ttotalSalidas" style="margin: 5px 0; font-weight: bold; color: #d32f2f;">Total de Salidas: ${totalSalidasGlobal.toLocaleString(
        "en-US",
        { minimumFractionDigits: 2, maximumFractionDigits: 2 }
      )}</p>
    </div>
...
function armarTablaDetalleTrasladosVerificados(desde, hasta) {
  ...
  const headers = headersOriginales.filter(
    (header) => header.toUpperCase() !== "DESCRIPCION"
  );
    ...
    if (ArrayDataFiltrado[i]) {
      ...
      headers.forEach((header) => {
        ...
        if (header.toUpperCase() === "ARTICULO") {
          ...
          if (
            header.toUpperCase() === "CANT TRASLADO" &&
            !isNaN(value) &&
            value !== ""
          ) {
  ...
  if (desde === 0) {
    // Ejecutar solo en la primera página para evitar recálculos innecesarios
