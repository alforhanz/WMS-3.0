      ...
      if (result.msg === "SUCCESS") {
        if (result.detalleOC.length != 0) {
          ...
          const siGuardadoParcial = detalleLineasOrdenDeCompra.some(
            (detalle) =>
              detalle.cant_verificada != null && detalle.cant_verificada !== ""
          );
...
function armarTablaLectura(detalleLineasOrdenDeCompra) {
  ...
  detalleLineasOrdenDeCompra.forEach(function (detalle) {
    ...
    if (!isNaN(cantidadVerificada) && cantidadVerificada > 0) {
      // Verificar que CANTIDAD_VERIFICADA sea mayor a 0
      ...
      newRow.innerHTML = `
              ...
              <td class="codigo-barras-cell2" style="text-align: center;">
                  <input id="cant-pedida" style="text-align: center;" type="text" class="codigo-barras-input" value="${
                    detalle.cant_verificada || ""
                  }" onchange="guardarTablaEnArray(this)" readonly>
              </td>
              <td class="codigo-barras-cell2" style="text-align: center;">
  ...
  if (lecturaSwitch) {
    for (var i = 0; i < LineasOrdenDeCompra.length; i++) {
      ...
      if (LineasOrdenDeCompra[i].codigos_barras) {
        codigosArrayArticulo = LineasOrdenDeCompra[i].codigos_barras
          .split("|")
          .map((codigo) => codigo.toUpperCase());
        codigoArrayPadre = LineasOrdenDeCompra[i].codigos_barras_kits
          .split("|")
          .map((codigo) => codigo.toUpperCase());
      }

      if (
        (LineasOrdenDeCompra[i].articulo &&
          LineasOrdenDeCompra[i].ARTICULO.toUpperCase() === codbarra) ||
        codigosArrayArticulo.includes(codbarra) ||
        codigoArrayPadre.includes(codbarra)
      ) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          Swal.fire({
            ...
            preConfirm: () => {
              const usuario =
                Swal.getPopup().querySelector("#usuariocodigo").value;
              const contrasena =
                Swal.getPopup().querySelector("#contrasena").value;
              const articulo = Swal.getPopup().querySelector("#articulo").value;
              const codigoBarra =
                Swal.getPopup().querySelector("#codigoBarra").value;
            ...
            if (result.isConfirmed) {
              const { usuario, contrasena, articulo, codigoBarra } =
                result.value;
              InsertaCodigoBarra(
                usuario,
                contrasena,
    ...
    for (var i = 0; i < LineasOrdenDeCompra.length; i++) {
      ...
      let codigoArrayHijo = [];
      if (LineasOrdenDeCompra[i].codigos_barras) {
        codigosArrayPadre = LineasOrdenDeCompra[i].codigos_barras_kits
          .split("|")
          .map((codigo) => codigo.toUpperCase());
        codigoArrayHijo = LineasOrdenDeCompra[i].codigos_barras
          .split("|")
          .map((codigo) => codigo.toUpperCase());
      }
      ...
      if (
        (LineasOrdenDeCompra[i].articulo &&
          LineasOrdenDeCompra[i].ARTICULO_PADRE.toUpperCase() === codbarra) ||
        codigosArrayPadre.includes(codbarra) ||
        codigoArrayHijo.includes(codbarra)
      ) {
...
function verificacion() {
  ...
  filas.forEach((lineas) => {
    ...
    dataArray.forEach((arrayLectura) => {
      //recorremos el arreglo de las cantidades leidas
...
function confirmarGuardadoParcialLectura() {
  Swal.fire({
    icon: "info",
    title:
      "¿A continuación se guardaran los datos leidos de la Orden De Compra...?",
    showCancelButton: true,
...
function validarVerificacion() {
  ...
  var celdasVerificacion = document.querySelectorAll(
    "#tblbodyLineasOrdenDeCompra td#verificado"
  );
          ...
          if (result.isConfirmed) {
            if (observaciones != "") {
              ...
              Swal.fire({
                icon: "warning",
                title:
                  "Debe ingresar las observaciones antes de procesar la orden de compra",
                showCancelButton: true,
                confirmButtonText: "Continuar",
                cancelButtonText: "Cancelar",
                confirmButtonColor: "#28a745",
...
function procesarOrdenDeCompra() {
  ...
  for (let i = 0; i < totalBatches; i++) {
    ...
    const fetchPromise = fetch(
      env.API_URL + "wmsguardaprocesaordendecompralist/P" + params,
      myInit
    )
      .then((response) => response.json())
      .then((result) => {
...
document
  .getElementById("toggleSwitchLectura")
  .addEventListener("change", saveSwitchState);
