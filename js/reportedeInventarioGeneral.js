...
async function resumenGeneral() {
  ...
  const pSoloContados = document.getElementById("articulosContados").checked
    ? "S"
    : "N";
  const pFecha = document.getElementById("fecha_ini").value;
  const pBodega = document.getElementById("bodega").value;

  let pAgrupadoClase = document.getElementById("agrupadoClase").checked
    ? "S"
    : "N";
  let pAgrupadoMarca = document.getElementById("agrupadoMarca").checked
    ? "S"
    : "N";

  const pclase =
    document.getElementById("claseReporte").value.trim() === ""
      ? null
      : document.getElementById("claseReporte").value;
  const pmarca =
    document.getElementById("marcaReporte").value.trim() === ""
      ? null
      : document.getElementById("marcaReporte").value;
  const ptipo =
    document.getElementById("tipoReporte").value.trim() === ""
      ? null
      : document.getElementById("tipoReporte").value;
  const penvase =
    document.getElementById("envaseReporte").value.trim() === ""
      ? null
      : document.getElementById("envaseReporte").value;
  const pventa =
    document.getElementById("ventasReporte").value.trim() === ""
      ? null
      : document.getElementById("ventasReporte").value;
  const pseis =
    document.getElementById("seisReporte").value.trim() === ""
      ? null
      : document.getElementById("seisReporte").value;
...
function limpiarTabla() {
  const tablas = [
    ...
    {
      id: "tablaReporteRCM",
      tbody: "tablaReporteRCM",
      div: "reporteInventarioRCM",
    },
    {
      id: "tablaReporteRM",
      tbody: "tablaReporteRM",
      div: "reporteInventarioRM",
    },
    {
      id: "tablaReporteRC",
      tbody: "tablaReporteRC",
      div: "reporteInventarioRC",
    },
...
function generarReporteInventarioRCM(datos) {
  ...
  let granTotalSistema = 0,
    granTotalConteo = 0,
    granTotalDiferencia = 0,
    granTotalCantSistema = 0,
    granTotalCantContada = 0;
  ...
  Object.entries(marcasOrdenadas).forEach(([marca, items]) => {
    let subtotalSistema = 0,
      ...
      subtotalCantSistema = 0,
      subtotalCantContada = 0;
...
function generarReporteInventarioD(datos) {
  ...
  let subtotalBodegaSistema = 0,
    subtotalBodegaConteo = 0,
    subtotalBodegaDiferencia = 0,
    subtotalBodegaCantSistema = 0,
    subtotalBodegaCantContada = 0;
  ...
  Object.entries(clasesAgrupadas).forEach(([clase, marcas]) => {
    ...
    let subtotalClaseSistema = 0,
      subtotalClaseConteo = 0,
      subtotalClaseDiferencia = 0,
      subtotalClaseCantSistema = 0,
      subtotalClaseCantContada = 0;
...
function descargarPDFRCM() {
  ...
  const tabla = document
    .getElementById("reporteInventarioRCM")
    .querySelector("table");
  ...
  const dibujarEncabezado = () => {
    ...
    doc.text(
      `Bodega: B-${pBodega}  /  Fecha del inventario: ${fechaInventario}`,
      9,
      50
    );
...
function descargarPDFRM() {
  ...
  // Obtener las cabeceras de la tabla
  const tabla = document
    .getElementById("reporteInventarioRM")
    .querySelector("table");
  const thead = tabla.querySelector("thead");
  const headers = Array.from(thead.querySelectorAll("th")).map(
    (th) => th.textContent
...
function descargarPDFRC() {
  ...
  // Obtener las cabeceras de la tabla
  const tabla = document
    .getElementById("reporteInventarioRC")
    .querySelector("table");
  const thead = tabla.querySelector("thead");
  const headers = Array.from(thead.querySelectorAll("th")).map(
    (th) => th.textContent
...
function descargarPDFDetalle() {
  ...
  doc.text("CENTRAL DE LUBRICANTES, S.A.", pageWidth / 2, y, {
    align: "center",
  });
  ...
  doc.setFontSize(12);
  doc.text(
    "Reporte de Inventario General Detallado Por Bodega, Clase y Marca",
    pageWidth / 2,
    y,
    { align: "center" }
  );
  ...
  doc.text(
    "Fecha inventario: 2024-02-07 " +
      "Bodega: " +
      bodega +
      " / Clase y Marca, segun artículos contados",
    margenIzquierdo,
    y
  );
