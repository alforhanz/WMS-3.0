...
async function presentarBoleta() {
  ...
  let SoloDiferencia = document.getElementById("soloDiferencias").checked
    ? "S"
    : "N";
  let SoloConteoCero = document.getElementById("contIgualaCero").checked
    ? "S"
    : "N";
      ...
      if (result.msg === "SUCCESS") {
        ...
        if (result.resultado.length > 0) {
          ...
          Swal.fire({
            ...
            title: "Información",
            text:
              "No se puede proceder a crear la Boleta porque existen " +
              result.resultado.length +
              " artículos en estado Remitida.",
...
function limpiarTabla() {
  const tablas = [{ id: "myTableBoleta", tbody: "tblbodyBoleta" }];
